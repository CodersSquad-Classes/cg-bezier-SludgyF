...
def draw_bezier_curve(debug_image, ctrl_points):
    ...
    # Definir la cantidad de muestras para crear la curva suave
    num_steps = 1000

    for t in np.linspace(0, 1, num_steps):
        bezier_point = np.zeros(2)

        # Iterar sobre cada punto de control
        for i in range(n + 1):
            bernstein = coeffs[i] * (t ** i) * ((1 - t) ** (n - i))
            bezier_point += bernstein * np.array(ctrl_points[i])

        bezier_point = tuple(bezier_point.astype(int))

        cv.circle(debug_image, bezier_point, 1, (255, 0, 255), -1)
